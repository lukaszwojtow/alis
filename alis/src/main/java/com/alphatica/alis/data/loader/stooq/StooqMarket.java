...
public class StooqMarket implements Market {
    ...
    public Optional<TimeMarketData> getAtOrPrevious(Time time) {
        return ofNullable(ranges.floorEntry(time)).map(entry ->
                new TimeMarketData(name, marketType, entry.getKey(), entry.getValue())
        );
    }

    @Override
    public Optional<TimeMarketData> getAtOrNext(Time time) {
        return ofNullable(ranges.ceilingEntry(time)).map(entry ->
                new TimeMarketData(name, marketType, entry.getKey(), entry.getValue())
        );
    }
